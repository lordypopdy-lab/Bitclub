import { Link } from "react-router-dom";
import blog1 from "../../images/blog/blog1.jpg";
import blog2 from "../../images/blog/blog2.jpg";
import blog3 from "../../images/blog/blog3.jpg";
import blog4 from "../../images/blog/blog4.jpg";
import blog5 from "../../images/blog/blog5.jpg";
import blog6 from "../../images/blog/blog6.jpg";

const ListBlog = () => {
  return (
    <>
      <div className="header fixed-top bg-surface d-flex justify-content-center align-items-center">
        <Link to="javascript:void(0);" className="left back-btn"><i className="icon-left-btn"></i></Link>
        <h3>Hot News</h3>
        <Link to="#filter" className="right text-secondary" data-bs-toggle="modal"><i className="icon-funnel"></i></Link>
    </div>
    <div className="pt-45 pb-16">
        <div className="tf-container">
            <ul className="mt-12 grid-2 gap-16">
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog1} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Daria Morgen &#8226; Dec 21, 2023 </p>
                            <p className="mt-4">The Nightmare Before Christmas - Bomb Threats and Bitcoin</p>
                        </div>
                    </Link>
                </li>
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog2} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Jane Cooper &#8226; May 19, 2023 </p>
                            <p className="mt-4">The Crypto.com App Introduces Trusted Device Management</p>
                        </div>
                    </Link>
                </li>
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog3} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Wade Warren &#8226; June 9, 2023 </p>
                            <p className="mt-4">Cybersecurity Predictions 2023 – How did we do?</p>
                        </div>
                    </Link>
                </li>
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog4} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Esther Howard &#8226; Jan 27, 2023 </p>
                            <p className="mt-4">Cryptocurrency News: Building in Resiliency and International Co-Operation</p>
                        </div>
                    </Link>
                </li>
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog5} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Leslie Alex &#8226; Apr 19, 2023 </p>
                            <p className="mt-4">Blockchain, cryptocurrencies, and mining</p>
                        </div>
                    </Link>
                </li>
                <li>
                    <Link to="/BlogDetail" className="blog-item">
                        <img src={blog6} alt="img" />
                        <div className="content">
                            <p className="text-xsmall text-secondary title">Jenny Wilson &#8226; May 21, 2023 </p>
                            <p className="mt-4">Security Predictions 2023: how are we doing so far?</p>
                        </div>
                    </Link>
                </li>
                
            </ul>
            
        </div>
    </div>

    <div className="modal fade action-sheet" id="filter">
        <div className="modal-dialog" role="document">
            <div className="modal-content">
                <div className="modal-header">
                    <span>Filters</span>
                    <span className="icon-cancel" data-bs-dismiss="modal"></span>
                </div>
                <div className="modal-body">
                    <div className="text-small text-white">Time</div>
                    <ul className="grid-2 rcg-12-16 mt-16">
                        <li><Link to="javascript:void(0);" className="tf-btn xs line active text-secondary item-time">All</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-time">1 Hour</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-time">24 Hour</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-time">This week</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-time">This Month</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-time">NFT</Link></li>
                    </ul>
                    <div className="text-small text-white mt-16">Categories</div>
                    <ul className="grid-2 rcg-12-16 mt-16">
                        <li><Link to="javascript:void(0);" className="tf-btn xs line active text-secondary item-category">All</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-category">Bitcoin</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-category">ETH</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-category">NFT</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-category">Crypto</Link></li>
                        <li><Link to="javascript:void(0);" className="tf-btn xs line text-secondary item-category">NFT</Link></li>
                    </ul>
                    <div className="mt-16 pt-16 line-t grid-2 gap-16">
                        <Link to="javascript:void(0);" className="tf-btn sm secondary fw-4" data-bs-dismiss="modal">Reset</Link>
                        <Link to="javascript:void(0);" className="tf-btn sm primary fw-4" data-bs-dismiss="modal">Apply</Link>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
    </>
  )
}

export default ListBlog