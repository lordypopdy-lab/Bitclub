      ...
      if (trx.status == "Paused") {
        ...
        if (data.success) {
          ...
          if (receipt) {
            ...
            const { data } = await axios.post("/api/pauseContractOne", {
              email,
            });
            if (data.success) {
      ...
      <div class="pt-45 pb-16">
        <div class="tf-container">
          ...
          <p class="mt-8 text-primary">
            ${convertedPrice !== null && convertedPrice.toFixed(2)}+
            {convertedProfit !== null && convertedProfit.toFixed(2)} USD
          </p>
          <i>
            <Link
          ...
          <div className="tab-pane fade active show" id="link" role="tabpanel">
            <ul className="mt-10 accent-box line-border">
              <h5 className="text-primary">Activation History!</h5>
              <hr />
              ...
              <li className="trade-list-item">
                ...
                <p className="d-flex gap-8 text-white">
                  {trx.contractPrice !== null && trx.contractPrice.toFixed(4)}{" "}
                  ETH ={" "}
                  {trx.contractPrice !== null && convertedPrice.toFixed(2)} USDC{" "}
              ...
              <li className="trade-list-item mt-16">
                {trx.status == "Paused" ? (
                  <p className="d-flex align-items-center text-small gap-4">
                    Status<i className="icon-clock fs-16 text-warning"></i>{" "}
                  </p>
                ) : (
                  <p className="d-flex align-items-center text-small gap-4">
                    Status<i className="icon-check fs-16 text-primary"></i>{" "}
                  </p>
                )}
                {trx.status == "Paused" ? (
                  <span className="text-warning">{trx.status}</span>
                ) : (
                  <span className="text-success">Contract {trx.status}!</span>
                )}
              </li>
              {/* <Link className="tf-btn lg mt-20 primary" data-bs-toggle="modal" data-bs-target="#pause">Pause & Withdraw</Link> */}
            </ul>
          </div>
        </div>
...
export default WithdrawContractOne;
